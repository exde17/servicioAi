...
# GENERAL
def mediapipe_detection(image, model):
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # Convertimos la imagen a RGB
    image.flags.writeable = False  # Optimizamos la imagen para el procesamiento
    results = model.process(image)  # Detección con MediaPipe
    image.flags.writeable = True  # Volvemos la imagen editable
    image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)  # Convertimos de vuelta a BGR para OpenCV
    return image, results
...
# CREATE KEYPOINTS
def extract_keypoints(results):
    # Solo conservamos las manos y la pose
    pose = np.array([[res.x, res.y, res.z, res.visibility] for res in results.pose_landmarks.landmark]).flatten() if results.pose_landmarks else np.zeros(33*4)
    lh = np.array([[res.x, res.y, res.z] for res in results.left_hand_landmarks.landmark]).flatten() if results.left_hand_landmarks else np.zeros(21*3)
    rh = np.array([[res.x, res.y, res.z] for res in results.right_hand_landmarks.landmark]).flatten() if results.right_hand_landmarks else np.zeros(21*3)
...
def get_actions(path):
    """
    Obtiene una lista de nombres de acciones (palabras/señas) desde el directorio especificado.
    Solo devuelve archivos con la extensión .h5.
    """
    out = []
    for action in os.listdir(path):
        name, ext = os.path.splitext(action)
...
def format_sentences(sent, sentence, repe_sent):
    """
    Formatea las oraciones y maneja repeticiones de palabras/señas
    """
    if len(sentence) > 1:
        if sent in sentence[1]:
            repe_sent += 1
...
def save_txt(file_name, content):
    ...
    :param file_name: El nombre del archivo donde se guardará el contenido
